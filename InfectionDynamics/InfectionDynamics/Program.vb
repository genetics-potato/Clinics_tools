...
Module Program
    ...
    Sub Main()
        ...
        Dim fU As New ODEs.ODE With {.df = AddressOf dU, .SetY = Sub(x) U = x, .y0 = U0} ' The initial number of susceptible cells(U0) can be taken from the experiment in Half mannetal. (2008) as 5 × 10^5.
        Dim fI As New ODEs.ODE With {.df = AddressOf dI, .SetY = Sub(x) I = x, .y0 = 0R}    ' The initial value for infected cells(I0) is set to zero.
        Dim fV As New ODEs.ODE With {.df = AddressOf dV, .SetY = Sub(x) V = x, .y0 = 9}  ' The best model based on the Akaike Information Criterion(AIC) is presented in Figure3, providing an estimate of 9 ffu/ml for V0.
